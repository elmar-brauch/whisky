...
class WhiskyCrawler {
    ...
    private fun searchWhiskies(whiskyName: String): Map<String, List<String>> {
        ...
        for(whisky in browser.findElementsByXPath("//div[@class='row']//a[@class='clickable']"))
            map.putIfAbsent(whisky.text, mutableListOf(whisky.getAttribute("href")))
                ?.add(whisky.getAttribute("href"))

        try {
            browser.findElementByXPath("//button[@name='allow']")?.click()
        } catch (e: Exception) {
    ...
    private fun readWishCollectionPair(url: String): Pair<Int, Int> {
        browser[url]
        try {
            val wish = browser.findElementByXPath("(//div[@id='whisky-community']//button)[1]")
            val coll = browser.findElementByXPath("(//div[@id='whisky-community']//button)[3]")
